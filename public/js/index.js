...
function adjustSalary(salary) {
  ...
  if (page === 7) {
    ...
    let monthly = document.getElementById("monthlySalary");
    let weekly = document.getElementById("weeklySalary");
    let daily = document.getElementById("dailySalary");

    userSalary *= taxRate;
    monthly.textContent = `Monthly: ${(userSalary / 12).toFixed(2)}`;
    weekly.textContent = `Weekly: ${(userSalary / 52).toFixed(2)}`;
    daily.textContent = `Daily: ${(userSalary / 260).toFixed(2)}`;
  }

  salary.textContent = `Annually: ${userSalary}`;
}
